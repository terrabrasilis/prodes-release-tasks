...
do
    ...
    TABLES=("border" "marco_eu_deforestation" "no_forest" "hydrography" "accumulated" "yearly" "residual")
    # for amazonia we must include data for no forest areas
    if [[ "${TARGET_NAME}" = "amazonia" ]];
    then
        TABLES+=("marco_eu_deforestation_nf" "hydrography_nf" "accumulated_nf" "yearly_nf" "residual_nf")
    fi;
    ...
    do
        ...
        then
            ...
            # if [[ -f "${OUTPUT_DIR}/${MARCO_OUTPUT_FILE}.tif" ]];
            # then
            #     # store the generated file into MARCO input list used in next step
            #     MARCO_INPUT_FILES+=("${MARCO_OUTPUT_FILE}.tif")
            # fi;
...
# remove the temporary files (style fractions and vrt) to clean the output dirs
